...
neurons = {
    [DifferentialNeuron(0, rand()*scale + minStartWeight, dt, memorySize) DifferentialNeuron(1, rand()*scale + minStartWeight, dt, memorySize) DifferentialNeuron(2, rand()*scale + minStartWeight, dt, memorySize)]};


M = 7;
K = 2;
b = .2;
x = exp(-b*t/(2*M)) .* cos(((sqrt(4*M*K)-b^2)/(2*M))*t);
        ...
        for t = 1:length(inputSeries)
            ...
            for layer = length(neurons):-1:1
                for i = 1:length(neurons{layer})
                    ...
                    % Backpropagated gradient for this neuron
                    gradient = error * neuron.lastOutput;
                    ...
                    % Update weight
                    if (neuron.weight - learningRate * gradient) >= 0
                        neuron.weight = neuron.weight - learningRate * (3/epoch) * gradient;
                    end
                    
                    % Update the error to propagate backward to previous layers
                    error = error * neuron.weight;
